...
class FrameParser:
    ...
    def __init__(self) -> None:
        ...
        self._loss_count = 0
        self._pending_sync_frames: list[ParsedFrame] = []
        self._ascii_buffer = ''
        ...
        while offset < len(buf):
            if self._sync_state in (SyncState.SEARCHING, SyncState.CONFIRMING_SYNC):
                ...
                if result is not None:
                    ...
                    offset = next_offset
                    if self._on_frame_found(mode, frame):
                        results.extend(self._pending_sync_frames)
                        self._pending_sync_frames = []
    ...
    def _on_frame_found(self, mode: ChecksumMode, frame: ParsedFrame) -> bool:
        """Track sync candidates. Returns True once sync is confirmed."""
        if self._sync_state == SyncState.SEARCHING:
            self._checksum_mode = mode
            self._confirm_count = 1
            self._pending_sync_frames = [frame]
            ...
            if (
                ...
                self._confirm_count += 1
                self._pending_sync_frames.append(frame)
    ...
    def _reset_sync_confirmation(self) -> None:
