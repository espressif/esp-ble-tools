...
class TrafficSpikeDetector:
    ...
    def __init__(self) -> None:
        ...
        self._spike_peak_bits_per_sec: float = 0.0
    ...
    def check(self) -> TrafficSpikeResult | None:
        ...
        wire_bits_per_sec = self._bitrate.wire_bits_per_sec
        if wire_bits_per_sec is None or wire_bits_per_sec <= 0:
            return None

        window_bytes = sum(b for _, b, _ in window)
        throughput_bits_per_sec = self._bitrate.payload_bytes_to_wire_bits(window_bytes) / TRAFFIC_WINDOW_SEC
        utilization = throughput_bits_per_sec / wire_bits_per_sec

        if utilization >= TRAFFIC_THRESHOLD_PCT:
            if not self._spike_active:
                self._spike_active = True
                self._spike_start = now
                self._spike_peak_bits_per_sec = 0.0
                self._spike_per_source = {}
                for _, b, src in window:
                    self._spike_per_source[src] = self._spike_per_source.get(src, 0) + b
            if throughput_bits_per_sec > self._spike_peak_bits_per_sec:
                self._spike_peak_bits_per_sec = throughput_bits_per_sec
            return None
        ...
        spike_bits_per_sec = self._spike_peak_bits_per_sec
        ...
        return TrafficSpikeResult(
            throughput_bits_per_sec=spike_bits_per_sec,
            wire_max_bits_per_sec=wire_bits_per_sec,
            utilization_pct=spike_bits_per_sec / wire_bits_per_sec * 100.0,
