    ...
    *,
    ...
    try:
        ...
        while not stop_requested.is_set():
            ...
            if not _put_parse(parse_queue, block) and not parse_backlog_reported:
                parse_backlog_reported = True
                _put_event(
                    event_queue,
                    ReaderProcessEvent(
                        kind='parse_backlog',
                        message='parse_queue full; realtime parsing is behind',
                        status=reader.status(),
                    ),
                )
            ...
            for block in reader.drain(drain_rounds):
                ...
                if not _put_parse(parse_queue, block) and not parse_backlog_reported:
                    ...
                    _put_event(
                        ...
                        ReaderProcessEvent(
                            kind='parse_backlog',
                            message='parse_queue full while draining transport',
                            status=reader.status(),
                        ),
                    )
    except Exception as e:
        ...
        if not _put_parse_sentinel(parse_queue, raw_put_timeout) and not parse_backlog_reported:
            _put_event(
                ...
                ReaderProcessEvent(
                    kind='parse_backlog',
                    message='parse_queue full while sending parser sentinel',
                    status=reader.status(),
                ),
            )
