...
_ZH_CN = {
    ...
    'Firmware log write failures (failed / total)': '固件日志写入失败（失败 / 总量）',
    'Firmware write failures in parsed portion (failed / total)': '已解析部分固件日志写入失败（失败 / 总量）',
    'Reliability': '可信范围',
    'Saved raw data': '已保存原始数据',
    'Complete and reliable': '完整且可信',
    'Incomplete': '不完整',
    'Firmware written bytes': '固件日志写入成功字节数',
    'Firmware lost bytes': '固件日志写入失败字节数',
    'Firmware observed total bytes': '固件日志统计总字节数',
