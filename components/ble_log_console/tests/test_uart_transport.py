...
class TestValidateUartPort:
    @patch('src.backend.transport.uart_transport.list_serial_ports', return_value=['/dev/ttyUSB0', '/dev/ttyUSB1'])
    def test_valid_port_returns_none(self, _mock: object) -> None:
        assert validate_uart_port('/dev/ttyUSB0') is None

    @patch('src.backend.transport.uart_transport.list_serial_ports', return_value=['/dev/ttyUSB0'])
    ...
    @patch('src.backend.transport.uart_transport.list_serial_ports', return_value=['COM3', 'COM4'])
    ...
    @patch('src.backend.transport.uart_transport.list_serial_ports', return_value=[])
