...
class TestFrameParserOutput:
    ...
    def test_redir_source_code_8_is_parsed(self) -> None:
        parser = FrameParser()
        frames_data = b''
        for sn in range(3):
            frames_data += _make_sum_frame(b'console line\n', src=BleLogSource.REDIR, sn=sn)

        results = parser.feed(frames_data)
        parsed = [r for r in results if hasattr(r, 'source_code')]

        assert len(parsed) == 3
        assert all(f.source_code == BleLogSource.REDIR for f in parsed)

    def test_legacy_spi_out_layout_does_not_sync_as_new_frame(self) -> None:
        parser = FrameParser()
        payload = b'\x01\x02\x03\x04payload'
        legacy_frames = b''
        for sn in range(3):
            legacy_header = len(payload).to_bytes(2, 'little') + bytes([1, 0]) + sn.to_bytes(4, 'little')
            checksum = sum_checksum(legacy_header + payload).to_bytes(4, 'little')
            legacy_frames += legacy_header + payload + checksum

        results = parser.feed(legacy_frames)

        assert not [r for r in results if hasattr(r, 'source_code')]
        assert parser.sync_state == SyncState.SEARCHING
